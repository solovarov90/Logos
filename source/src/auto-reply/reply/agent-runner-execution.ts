...
export async function runAgentTurnWithFallback(params: {
  ...
  while (true) {
    try {
      ...
      if (
        (isTransientHttp || isRateLimitErrorMessage(message)) &&
        !didRetryTransientHttpError
      ) {
        didRetryTransientHttpError = true;
        const retryDelayMs = parseRetryDelay(message) ?? TRANSIENT_HTTP_RETRY_DELAY_MS;
        const safeDelayMs = retryDelayMs + 1000; // Add 1s buffer

        defaultRuntime.error(
          `Transient error before reply (${message}). Retrying once in ${safeDelayMs}ms.`,
        );
