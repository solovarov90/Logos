...
export function resolveModel(
  ...
  // Force resolve Google Gemini 1.5 Flash if not found in registry (temporary fix for startup race/registry lag)
  if (!model && (provider === "google-generative-ai" || provider === "google")) {
    const normalizedId = modelId.replace("google/", "");
    if (normalizedId === "gemini-1.5-flash-001" || normalizedId === "gemini-1.5-flash") {
       return {
