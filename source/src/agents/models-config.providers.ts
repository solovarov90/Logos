...
// Groq offers Llama 3 models for free (beta).
// Costs are effectively 0 for now, but we'll use placeholder values or 0.
...
const GROQ_DEFAULT_CONTEXT_WINDOW = 4096; // Reduced to fit within free tier TPM limits (6k TPM)
const GROQ_DEFAULT_MAX_TOKENS = 8192; // Typical output limit
  ...
  return {
    ...
    models: [
      ...
      },
      {
        ...
        reasoning: false,
        input: ["text"], // Groq supports audio via separate endpoint, but model config type restricts to text/image
        cost: GROQ_DEFAULT_COST,
        contextWindow: 0,
        maxTokens: 0,
      },
