...
export const OPENCODE_ZEN_MODEL_ALIASES: Record<string, string> = {
  ...
  // Legacy Gemini 3 aliases (map to current).
  "gemini-3": "gemini-1.5-pro",
  "gemini-3-pro": "gemini-1.5-pro",
  "gemini-3-flash": "gemini-1.5-flash",

  // Legacy Gemini 2.5 aliases
  "gemini-2.5": "gemini-1.5-pro",
...
> = {
  ...
  "gemini-1.5-pro": { input: 3.5, output: 10.5, cacheRead: 0.875, cacheWrite: 0 },
  "gpt-5.1-codex-mini": {
    input: 0.25,
    output: 2,
    cacheRead: 0.025,
  ...
  "gemini-1.5-flash-001": { input: 0.075, output: 0.3, cacheRead: 0.01875, cacheWrite: 0 },
  "gpt-5.1-codex-max": {
    input: 1.25,
    output: 10,
    cacheRead: 0.125,
...
function buildModelDefinition(modelId: string): ModelDefinitionConfig {
  if (modelId === "google/gemini-1.5-flash-001" || modelId === "gemini-1.5-flash-001") {
    return {
...
function resolveModelInputs(modelId: string): string[] {
  const lower = modelId.toLowerCase();
  const inputs = ["text"];

  // Image support (exclude text-only models)
  if (!lower.includes("glm") && !lower.includes("minimax")) {
    inputs.push("image");
  }

  // Audio support (Gemini models)
