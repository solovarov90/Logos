...
async function runExecProcess(opts: {
  ...
  // FIX: Force UTF-8 encoding for PowerShell on Windows to prevent garbled text (krakozyabry)
  if (process.platform === "win32" && !opts.sandbox) {
    opts.command = `$OutputEncoding = [Console]::InputEncoding = [Console]::OutputEncoding = New-Object System.Text.UTF8Encoding; ${opts.command}`;
  }
