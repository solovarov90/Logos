...
const RETRY_DELAY_RE = /retry in (\d+(?:\.\d+)?)s/i;

export function parseRetryDelay(raw: string): number | undefined {
  if (!raw) {
    return undefined;
  }
  const match = raw.match(RETRY_DELAY_RE);
  if (match?.[1]) {
    const seconds = Number.parseFloat(match[1]);
    if (Number.isFinite(seconds)) {
      return Math.ceil(seconds * 1000); // Return milliseconds
    }
  }
  return undefined;
}
